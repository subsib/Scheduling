...
\input{content/intro/presentation_subject}	
	\section{Problématique : Choix de l'ordonnanceur}
	...
	La littérature scientifique -- comme nous le verrons dans l'état de l'art -- 
	est rarement explicite quant à l'implémentation. Les ordonnanceurs proposés sont 
	généralement présentés de façon très théorique et sans aide pour la mise en œuvre. 
	Il est important de choisir un ordonnanceur qui pourra nous permettre de tirer 
	des conclusions utiles. Par exemple : 
	Que devrait comporter la littérature scientifique afin de permettre l'implémentation 
	de ses algorithmes ? À quels détails un chercheur souhaitant proposer un nouvel algorithme 
	devrait-il penser pour faciliter le passage de son travail théorique à une implémentation 
	usuelle ?
	
	En outre, certains articles citent des avantages d'un algorithme, comme par exemple 
	le nombre réduit de migration, ou de préemptions attendues. Ces attentes étant théoriques, il est intéressant 
	de confronter les résultats observés avec ceux attendus. La mise en oeuvre nécessitant parfois quelques 
	accommodements voire choix stratégiques dans le choix des structures de données, boucles, etc. peut 
	modifier les attentes que l'on a "en théorie".
	...
	\todo{là bah faudra avoir des chiffres ... :/}
