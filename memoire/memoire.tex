...
\begin{document}
	...
	\begin{titlepage}
		...
		\begin{center}
			...
			{\Huge \par}
			\begin{center}{\LARGE Arabella Brayer\\ \emailArabella }\end{center}{\Huge \par}
	...
	\begin{abstract}
		abstract-text
		\customhighlight{test de texte hightlighté}
		\customItalic{Pis là, un peu d'italic purple}
	\end{abstract}
	\selectlanguage{francais}
	...
	\todo{change le toujours pour autre chose, c'est mal de dire toujours}
	...
	gaspillage tant au niveau de l'espace que du câblage. \todo{parler weight, power, cost, ?} 
	...
	principalement due à des améliorations physiques des composants. \todo{la formulation laisse entendre qu'une autre raison en est responsable...}
	...
	\begin{itemize}
		\item \customhighlight{Temps de réalisation}\label{tempsderealisation} : C'est le moment $t$ où une tâche $\tau$ peut commencer à être exécutée.
		\item \customhighlight{Temps de réponse\label{Response Time}} : Temps maximum nécessaire à la réalisation de la tâche 
		entre le temps de réalisation et Fin.
		\item \customhighlight{Worst Case Execution Time}\label{wcet} : Afin de faciliter les calculs et l'abstraction du problème, l'on pose habituellement que le temps considéré d'exécution de 
		...
		est  \customhighlight{WCET} : \customhighlight{W}orst \customhighlight{C}ase \customhighlight{E}xecution \customhighlight{T}ime
		
		\item \customhighlight{Laxité} : La laxité d'une tâche est la durée entre sa réalisation et son échéance. 
		Pour une tâche $\tau_i$, la laxité est :
		\begin{center}
			$L_i = D_i - C_i$
		\end{center}
		\vspace{0.5cm}
		\item \customhighlight{Tâches périodiques} :
		...
		\begin{itemize}
			\item \customhighlight{O} : est l'"offset", c'est à dire le temps que met la tâche à générer un premier job.
			\item \customhighlight{T} : est la période, c'est à dire le temps qui sépare deux générations de job par la tâche. 
			...
			\item \customhighlight{D} est l'échéance relative, c'est à dire le temps qui sépare au maximum la génération 
			d'un job et sa réalisation.
			\item \customhighlight{C} est le temps de réalisation. Dans les algorithmes, on utilise -- comme expliqué précédemment -- le \customhighlight{WCET}.
		\end{itemize}	
		...
		\item \customhighlight{Tâche sporadique} : Une tâche sporadique est une tâche qui génère de nouveaux jobs, 
