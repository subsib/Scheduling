...
\todo{ajouter surcout dans le glossaire}
	...
	Dans la famille des ordonnanceurs globaux se trouve également \textbf{Global-FTP} [\ref*{globalftp}]\todo{présenter globalFTP dans état de l'art}, 
...
\end{algorithm}	
	...
	Et pour s'assurer de la faisabilité de l'ensemble de tâches, 
	il faudra tester l'ordonnancement suffisamment longtemps (hyper-période).\newline

	Dans la littérature présentant U-EDF, on parle de généralisation d'EDF de façon 
	verticale pour Global-EDF, ou de généralisation horizontale pour U-EDF. 
	Cette image peut se comprendre d'autant mieux avec le schéma suivant :
	\todo{insérer schema avec procs verticaux ou horizontaux}
...
\section{Difficultés liées à l'implémentation}
	...
	\subsubsection{Utilisation instantanée}
		...
		\todo{trouver bonne explmiucation, je crois que j'ai compris}.
