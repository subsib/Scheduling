...
\todo{ajouter surcout dans le glossaire}
	...
	Dans la famille des ordonnanceurs globaux se trouve également \textbf{Global-FTP} [\ref*{globalftp}]\todo{présenter globalFTP}, 
	néanmoins pour ce dernier, les priorités ont un fonctionnement très différent 
	et il nous semblait moins approprié de les comparer. 
	...
	\textbf{Global-ED}F peut être considéré comme \og{}vertical\fg{} là où UEDF serait \og{}horizontal\fg{}, et ce faisant, 
	...
	L'algorithme est simple, et pour les besoins de la comparaison, nous rappelons 
...
\end{algorithm}	
	...
	et celui-ci mettra en échec \textbf{Global-EDF} très facilement.\newline
	...
	va permettre de fournir toujours en tête la tâche de priorité supérieure.\newline

	Comparativement, nous caractérisons le fonctionnement d'UEDF d'\og{}horizontal\fg{} 
	car au lieu de remplir en priorité les processeurs \og{}libres\fg{}, \textbf{UEDF } 
	va remplir l'occupation des processeurs un par un. 
	\todo{insérer schema},
...
\section{Difficultés liées à l'implémentation}
	...
	\subsubsection{Utilisation instantanée}
		...
		Dans \textbf{HIPPEROS}, il existe d'autres ordonnanceurs déjà implémentés. 
		Ils ont leur fonctionnement propre, et de façon générale, sont assez différents d'\textbf{UEDF} par 
		rapport aux variables auxquelles ils accèdent. Si l'\textit{utilisation} [\ref*{utilisation}] d'une tâche n'est pas une 
		donnée très difficile à produire, en revanche, l'utilisation instantanée [\ref*{instantutilisation}] d'un système 
		pose certaines questions, comme celle-ci : comment arrondir les valeurs 
		(de combien peut-on décaler afin d'avoir le moins de pertes possibles). 
		La réponse à apporter diverge en fonction de plusieurs facteurs :\newline
		\begin{itemize}
			\item Selon les cas, il n'est pas nécessaire d'y faire appel (classes périodiques)
			\item Comment sont stockées les valeurs temporelles $period$ et $wcet$ ?
			\item Si l'on a un nombre plausible de tâches, quelle valeur pouvons-nous atteindre ?
		\end{itemize}
		Dans le cas présent, nous avons estimé que décaler les valeurs de $1000$ était suffisant. 
		Sur une autre plateforme, cette réponse pourrait être différente.
	...
	\subsection{Structures de données}
		...
		nous ont amené à reconsidérer ce choix. \textbf{UEDF} conserve un Heap, 
		qui est copié afin d'en faire le parcours ordonné au moment voulu. 
		Ce choix a fait diminuer drastiquement le surcoût. Néanmoins, 
		il serait tout aussi possible de conserver une liste de type \textbf{ArrayList}.
